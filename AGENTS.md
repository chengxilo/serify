...
- JSON byte-parity across languages is achievable but needs care: Go's `encoding/json` vs Rust's `serde_json` differ on `[]byte`→base64 and trailing `.0` on floats. See the hand-written `to_json`/`from_json` in `examples/rust/src/customer.rs` for how the Rust example matches Go exactly. Map-key ordering is no longer among the hazards — **workers must not sort map entries**, and a type holding a `map<K,V>` declares `oracle: semantic` so the decoded value is compared instead of the bytes (see `docs/protocol.md` § Maps: workers do not sort).
