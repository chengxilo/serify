...
1. **The runner** — the `serify` CLI (Go). `cmd/serify/` (cobra `main` package: `run`, `validate`, `schema`, `table`) → `internal/`:
   - `internal/conf` — parses per-type case YAML files and `worker.yaml` manifests.
   ...
   - `internal/proto` — the NDJSON wire format and `EncodeData` (test values → wire-safe forms).
   - `internal/orch` — runs the test rounds across all workers concurrently and compares results.
   - `internal/report` — the canonical result model + renderers (see below).
   - `internal/cmp` — hex and decoded-data diffs.
...
2. **The worker libraries** — what a worker author imports, one implementation per language:
   ...
   - `examples/taskstore/` — the **project-shaped** example, deliberately separate from the type zoo above: a small CRUD server over TCP. Go leads and writes the server; **all nine** languages ship a codec, a conformance worker and a socket client, so the eight followers are eight real clients rather than eight more workers. Its own `cases/` holds `request`/`response`, each a record wrapping a sum (`op`: list_all/create/read/update/delete; `result`: accepted/found/listing/failed), plus reusable `task`/`draft`/`task_page`/`api_error`; `_config.yaml` names go the leader so no `--ref` is needed, and every format is `oracle: bytes` (nothing here is a map, so there is no wire freedom to grant). The arrangement that makes it worth having: the conformance worker registers the **server's own** `MarshalBinary`, so the bytes the followers are checked against are the bytes the running server writes. `examples/taskstore/test/` runs both halves — the suite, and an end-to-end run that starts the real server on an ephemeral port and drives it with every client (the second is what would notice the server being rewired to a private encoder; every conformance cell would stay green). CI job: `taskstore`, which installs all nine toolchains and sets `SERIFY_REQUIRE`.
     - Two names are taken: **`Task`** shadows Elixir's built-in `Task` module (which breaks `mix` itself, since protocol consolidation calls `Task.async/1`) and C#'s `System.Threading.Tasks.Task` under `ImplicitUsings`. Both are `TaskItem`; a model's class name never reaches the wire.
     - Only **Go and Python** can keep the codec free of a serify dependency — inert struct tags, and applying `@serify_model` late from `worker.py`. Every other binding is a derive, attribute, annotation or macro and must be imported to compile. Rust's escape hatch is `default-features = false` (derive without the runtime).
     - The **Elixir** follower is the only one whose sum payload is not a struct: `serify_field(:op, :sum)` declares nothing, so the binding cannot know which module a struct payload belongs to and it arrives as a plain field map. The codec converts at the boundary with the model's own generated `to_field_map`/`from_field_map`.
     - The Rust follower is a **standalone crate** (its own empty `[workspace]`), like `test/cases/wrong/rust`, so its `worker` binary does not collide with `examples/appdata/rust`'s in the main Cargo workspace.
   - `examples/audit/` — the **idea-shaped** example, and the only user-facing demonstration of `--audit`. One type, three formats over an *identical* byte layout: `safe` copies, `fast` aliases the input buffer, `handoff` scrubs the caller's payload after writing. All three pass conformance (that is the point — the bytes are the same), and `--audit` reports zero-copy on `fast` and mutation *plus* instability on `handoff`. The double finding is worth understanding: `serializeAudit` re-calls the serializer to check stability (`lib/go/serify/run.go:102`), and because the scrubbed bytes are shared with the FieldMap the second call sees them — so this mutating serializer is an unstable one too.
     - Rust registers only `safe` and `fast` and reports SKIPPED for `handoff`: a Rust serializer takes `&FieldMap`, so the mutation cannot be written without an `unsafe` `&`→`&mut` cast. Go's `*FieldMap` makes the same bug a one-liner. `examples/audit/test/` asserts that exact cell rather than using `--expect-skips`, which has no per-format granularity by design (`internal/conf/conf.go:562`).
     - The Go worker registers a **model** (`Model: &Frame{}`, methods on `*Frame`) — what a worker author actually writes. Audit sees through it: `buildSerializer` snapshots the struct with `codec.extract` before and after the call, and the extracted FieldMap shares the struct's backing memory, so aliasing and mutation are caught exactly as at the raw boundary (`test/cases/audit/go` proves it for all nine detections).
       - `handoff` **scrubs** the payload (`clear(f.Payload)`) rather than reassigning `f.Payload = nil`. Both are mutations and audit reports either, but only the scrub is also *unstable*: the model path rebuilds the struct from the same FieldMap for the stability re-call, so a reassigned field is restored while scribbled-on shared bytes are not. Reassigning drops the run from 10 findings to 8 and silently kills the "one bug seen from both ends" point. Do not "simplify" it back.
     - The Rust worker registers a model as well, since `ModelFormat` was fixed to retain the instance and re-derive per probe. Before that it had to stay on `Format::new()`/`&FieldMap`, because the conversion cloned away the alias and all three `fast` findings vanished.
     - The `empty` and `no_payload` cases are deliberately quiet: there is nothing to alias in an empty string and nothing to release in an empty buffer. A finding depends on the data, not only the code — do not "fix" those silences.
...
- **The output-zero-copy probe reads the live model too, in every binding that has one** (Go, Rust, Python, Node, C#, Java). C# and Java lagged the other four here for a while: they routed the *mutation* check through the hook but left the flip comparing the caller's FieldMap, which happened to work only because `FromFieldMap` hands the model the same array rather than a copy. Reading the model directly costs nothing and stops the detection resting on that.
  - **No fixture reaches the C#/Java version of it, and none can.** The fault needs a model field that views a *sub-range* of the returned buffer, and a `byte[]` cannot: an array aliases another array only by being the same object. That is why `audit`'s `output-zero-copy` is Go and Rust only, and why `audit_model` does not carry the format at all — Rust cannot express it through a model either (it would need the same `&`→`&mut` cast that makes it decline `mutating`), and Go's `audit` type is already model-backed, so a second row would duplicate coverage rather than add it. A serialize-only format whose whole output *is* the payload would reach it (`return m.Payload` aliases by identity); that is a different byte layout from `audit`'s and a deliberate change to the fixture's design, not a gap to be closed silently.
...
the binding exposes `toFieldMap` as a static helper. All three were fixed for
`examples/taskstore`, whose `create`/`update`/`found` arms are exactly this
shape; `lib/node/test/`, `lib/php/test/` and the taskstore conformance run guard
them now. Check both directions when adding a tenth language. See "`sum` and your own types" in `docs/protocol.md` for the
...
- The `serify` CLI reads a **directory** of case files and auto-detects each worker's language from marker files in its directory. Case files are **YAML** (`.yaml`), loaded by `internal/conf` (`loadTypeFile`): each file holds a `fields:` (or `variants:`) section, a `formats:` list, an optional `import:` list for reusable named types, and `cases:`. The type name is the filename (without `.yaml`). Case data is decoded **schema-directed** (`casedata.go`): 64/128-bit integer fields are parsed exactly from their literal text (bare or quoted, any size), and loading rejects unknown fields, bad enum variants, float forms in integer fields, and out-of-range values.
...
- **64/128-bit integer case values are decoded schema-directed from their literal text** (`internal/conf/casedata.go`): bare literals of any size and quoted decimal strings are both exact; float forms (`1e3`) and out-of-range values are load-time errors. There is no quoting requirement.
...
- JSON byte-parity across languages is achievable but needs care: Go's `encoding/json` vs Rust's `serde_json` differ on `[]byte`→base64 and trailing `.0` on floats. See the hand-written `to_json`/`from_json` in `examples/appdata/rust/src/customer.rs` for how the Rust example matches Go exactly. Map-key ordering is no longer a hazard serify imposes: entry order is a property of the **format**, declared per (type, format) as `oracle: bytes` (canonical — the worker sorts explicitly) or `oracle: semantic` (free — the worker emits its own map's order). See `docs/protocol.md` § Maps: the format decides, not serify.

## Self-tests: the `wrong` meta-fixture (`test/cases/wrong`)

To verify that serify itself *reports* disagreements correctly (not just that honest workers agree), there is a deliberately-faulty `wrong` type — kept **out** of `examples/appdata/cases` so it never pollutes the real conformance suite:
...
| `lib/node/src/audit.ts` | Node lib: `detectZeroCopy`, `collectByteSnaps`, `dictDiffs` — a separate module rather than `workerlib.ts`, because that file is the package's `main` entry and exporting them there would be public npm API; the unit tests import `detectZeroCopy` from `dist/audit.js`. Audit in serialize/deserialize handlers stays in `workerlib.ts` |
...
go run ./cmd/serify run --ref rust --cases examples/appdata/cases examples/appdata/go examples/appdata/rust
