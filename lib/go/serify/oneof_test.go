...
func oneofSchema() []SchemaField {
	return []SchemaField{{
		Name: "id",
		Type: "oneof<balanced, numeric: uint32, name: string>",
...
func TestOneOf_DecodeEncodeRoundTrip(t *testing.T) {
	cases := []struct {
		name    string
		wire    string
		wantTag string
...
// A oneof payload is aliasing-capable, so the audit snapshot must deep-copy it.
...
func TestOneOf_SnapshotDeepCopiesPayload(t *testing.T) {
	fm := NewFieldMap()
	...
	if diffs := CompareFieldMaps(before, fm); len(diffs) != 1 || diffs[0] != "id" {
		t.Errorf("CompareFieldMaps = %v, want [id]: a mutated oneof payload must be detected", diffs)
	}
}

// DetectZeroCopy must see through a oneof: a bytes payload aliasing the input
// buffer is exactly the case iggy's partitioning<messages_key: bytes> hits.
func TestOneOf_DetectZeroCopyOnPayload(t *testing.T) {
	buf := []byte{1, 2, 3, 4}
	fm := NewFieldMap()
...
// A worker builds a oneof from scratch via SetVariant, and it encodes correctly.
func TestOneOf_SetVariantEncodes(t *testing.T) {
	fm := NewFieldMap()
	fm.SetVariant("id", "numeric", uint32(42))

	out, err := EncodeFieldMap(fm, oneofSchema())
	if err != nil {
		t.Fatalf("encode: %v", err)
	}
	got, _ := json.Marshal(out)
