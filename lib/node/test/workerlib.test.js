...
const { FieldMap, Serify, Variant, decodeFieldMap, encodeFieldMap,
        resolveRegistered, type } = require('../dist/workerlib.js');
const { detectZeroCopy } = require('../dist/audit.js');
...
// --- nested models -----------------------------------------------------------
//
// A nested struct cannot be recognised by `typeof val.toFieldMap === 'function'`:
// the binding exposes toFieldMap as a namespace function, not a method.

function makeNestedClasses() {
  class Inner {
    constructor() { this.city = ''; }
  }
  Serify.field()(Inner.prototype, 'city');
  Serify.Model()(Inner);
  ...
  Serify.field({ model: Inner })(Outer.prototype, 'home');
  Serify.field({ model: Inner })(Outer.prototype, 'seen');
  Serify.field({ model: Inner })(Outer.prototype, 'book');
  Serify.field({ model: Inner })(Outer.prototype, 'backup');
  Serify.Model()(Outer);
  return { Inner, Outer };
}

test('a nested model becomes a FieldMap, not a stringified object', () => {
  const { Inner, Outer } = makeNestedClasses();
  const o = new Outer();
  o.home.city = 'Toronto';

  const fm = Serify.toFieldMap(o);
  const inner = fm.getStruct('home');
  assert.ok(inner instanceof FieldMap, `home is ${inner}`);
  assert.equal(inner.getString('city'), 'Toronto');
});

test('a nested model comes back as its own class, not a raw FieldMap', () => {
  const { Inner, Outer } = makeNestedClasses();
  const o = new Outer();
  o.home.city = 'Toronto';

  const back = Serify.fromFieldMap(Outer, Serify.toFieldMap(o));
  assert.ok(back.home instanceof Inner);
  assert.equal(back.home.city, 'Toronto');
});

test('list<struct> and map<K,struct> convert in both directions', () => {
  const { Inner, Outer } = makeNestedClasses();
  const o = new Outer();
  const a = new Inner(); a.city = 'Leeds';
  const b = new Inner(); b.city = 'Zurich';
  o.seen = [a];
  o.book.set('work', b);

  const fm = Serify.toFieldMap(o);
  assert.ok(fm.getListStruct('seen')[0] instanceof FieldMap);
  assert.ok(fm.getMap('book').get('work') instanceof FieldMap);

  const back = Serify.fromFieldMap(Outer, fm);
  assert.ok(back.seen[0] instanceof Inner);
  assert.equal(back.seen[0].city, 'Leeds');
  assert.ok(back.book.get('work') instanceof Inner);
  assert.equal(back.book.get('work').city, 'Zurich');
});

test('an absent optional<struct> stays null in both directions', () => {
  const { Outer } = makeNestedClasses();
  const fm = Serify.toFieldMap(new Outer());
  assert.equal(fm.getOptionalStruct('backup'), null);
  assert.equal(Serify.fromFieldMap(Outer, fm).backup, null);
});
...
// The model binding does not classify a number — the schema does. These three
// pin that split: Number.isInteger would make every integral-valued float
// (0.0, -0.0, every float32 boundary) an integer.

test('a plain number reaches an integer field as its exact decimal string', () => {
  const User = makeUserClass();
  const u = new User();
  ...
  const schema = [{ name: 'id', type: 'uint64' }, { name: 'name', type: 'string' },
                  { name: 'email_addr', type: 'string' }];
  assert.equal(encodeFieldMap(Serify.toFieldMap(u), schema).id, '9007199254740991');
});

test('an integer field still rejects a number that cannot hold its value', () => {
  const User = makeUserClass();
  const u = new User();
  ...
  // The binding accepts it — it has no schema to judge against. The encoder
  // does, and that is where truncation has to be caught.
  const fm = Serify.toFieldMap(u);
  const schema = [{ name: 'id', type: 'uint64' }, { name: 'name', type: 'string' },
                  { name: 'email_addr', type: 'string' }];
  assert.throws(() => encodeFieldMap(fm, schema), /cannot hold it exactly/);
});

test('an integral-valued float is a float, not an integer', () => {
  const User = makeUserClass();
  const u = new User();
  u.id = 3.4028234663852886e38; // integral, unsafe as an integer, exact as float32
  u.name = '';
  u.email = '';

  const schema = [{ name: 'id', type: 'float32' }, { name: 'name', type: 'string' },
                  { name: 'email_addr', type: 'string' }];
  // ffff7f7f is float32 MAX. Classifying by value made this throw instead.
  assert.equal(encodeFieldMap(Serify.toFieldMap(u), schema).id, 'ffff7f7f');
});
...
test('detectZeroCopy sees an aliased sum payload', () => {
  ...
  // be reported, which needs collectByteSnaps to descend into the variant.
...
test('list supports every scalar element type', () => {
  // `serify validate` cannot catch a missing element type — the runner's
  // encoder is generic — so only a worker run would.
...
test('an unknown field type fails loudly in both directions', () => {
  // Falling off the chain leaves the field absent, or sends the value out
  // untouched: both surface far downstream rather than here.
...
// --- a sum arm carrying a model ----------------------------------------------
//
// toVariant converts a single model payload out to a FieldMap; fromVariant must
// rebuild the class, not hand back a raw FieldMap.

function makeArmClasses() {
  class Payload {
    constructor() { this.city = ''; }
    shout() { return this.city.toUpperCase(); }
  }
  Serify.field()(Payload.prototype, 'city');
  Serify.Model()(Payload);

  class Wrapped {
    constructor(value = new Payload()) { this.value = value; }
  }
  class Bare {}

  class Holder {
    constructor() { this.pick = new Bare(); }
  }
  Serify.sum([Bare, Wrapped])(Holder.prototype, 'pick');
  Serify.Model()(Holder);

  return { Payload, Wrapped, Bare, Holder };
}

test('a sum arm holding a model comes back as that model, not a FieldMap', () => {
  const { Payload, Wrapped, Holder } = makeArmClasses();
  const h = new Holder();
  const p = new Payload();
  p.city = 'Toronto';
  h.pick = new Wrapped(p);

  const back = Serify.fromFieldMap(Holder, Serify.toFieldMap(h));
  assert.ok(back.pick instanceof Wrapped);
  assert.ok(back.pick.value instanceof Payload, `payload is ${back.pick.value}`);
  assert.equal(back.pick.value.city, 'Toronto');
  // The methods are the point: a raw FieldMap has none of them.
  assert.equal(back.pick.value.shout(), 'TORONTO');
});

test('a sum arm holding a scalar is untouched by the model path', () => {
  const { Bare, Holder } = makeArmClasses();
  const h = new Holder();
  h.pick = new Bare();

  const back = Serify.fromFieldMap(Holder, Serify.toFieldMap(h));
  assert.ok(back.pick instanceof Bare);
});
