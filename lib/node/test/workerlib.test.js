...
// --- nested models -----------------------------------------------------------
//
// A nested struct used to be recognised by `typeof val.toFieldMap === 'function'`,
// which no model has ever satisfied: the binding exposes toFieldMap as a
// namespace function, not a method. Every nested struct therefore fell through
// to setString(key, String(val)) and arrived as "[object Object]". These four
...
// pin that split. It used to guess with Number.isInteger and store the result
// as a bigint, which made every integral-valued float (0.0, -0.0, and every
// float32 boundary) an integer, and rejected the large ones outright as
// unrepresentable while they were perfectly good floats.
...
//
// toVariant converted a single model payload out to a FieldMap, but fromVariant
// had no inverse: the arm's payload came back without the class's own methods.
