...
func TestCLI_Run_FullMatrix(t *testing.T) {
	...
	out, code := testutil.RunSerify(t, happy.runArgs(lang.Go, happy.CasePath(), "--full-matrix", "--csv", csv)...)
	...
	pairs := []string{lang.Go, lang.Rust}
	...
	testutil.AssertCell(t, grid, id, lang.Go, report.OpSerialize, report.StatusPass, nil)
	testutil.AssertCell(t, grid, id, lang.Rust, report.OpSerialize, report.StatusPass, nil)
}
...
func TestCLI_Run_JSONOutput(t *testing.T) {
	...
	out, code := testutil.RunSerify(t, happy.runArgs(lang.Go, happy.CasePath(), "--output", "json")...)
	require.Equal(t, 0, code, "exit code = %d, want 0\n%s", code, out)
	...
	for _, r := range records {
		if r.TestID == "all_types/json/basic" && r.Language == lang.Go && r.Operation == "serialize" &&
			r.Status == report.StatusPass {
			found = true
			break
		}
	}
	assert.True(t, found, "all_types/json/basic "+lang.Go+" serialize PASS not found in %d records", len(records))
}
...
func TestCLI_Run_MalformedCases(t *testing.T) {
	...
	out, code := testutil.RunSerify(t, slices.Concat(
		[]string{"run", "--ref", lang.Go, "--cases", badDir, "--no-build"},
		happy.WorkerPaths())...)
...
func TestCLI_Run_BadRunCommand(t *testing.T) {
	...
	out, code := testutil.RunSerify(t, "run", "--ref", lang.Go, "--cases", happy.CasePath(), "--no-build", tmp)
	require.NotEqual(t, 0, code, "expected non-zero exit, got 0\n%s", out)
	assert.Contains(t, out, "start "+lang.Go+" worker", "bad run command")
}
...
func TestCLI_Run_DuplicateLanguage(t *testing.T) {
	...
	out, code := testutil.RunSerify(t,
		"run", "--ref", lang.Go, "--cases", happy.CasePath(), "--no-build",
		goWorker, goWorker)
	require.NotEqual(t, 0, code, "expected non-zero exit for two workers of the same language\n%s", out)
	assert.Contains(t, out, "two "+lang.Go+" workers", "error should name the duplicated language")
}
